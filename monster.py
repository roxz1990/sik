...
async def start(update: Update, context: CallbackContext):
    ...
    message = (
        ...
        "* DM TO BUY 600sec bot :- @Roxz_gaming*"
    )
...
async def clear_logs_command(update: Update, context: CallbackContext):
    if update.effective_chat.id != ADMIN_USER_ID:
        await context.bot.send_message(chat_id=update.effective_chat.id, text="*⚠️JO KAL TAK HAMARE DUM PE UDTE THE ! VO AAJ MUJHE AASMAN KE BAARE ME BATA RAHE HAI 😂 .*", parse_mode='Markdown')
        return
...
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        ...
        attack_in_progress = False
        await context.bot.send_message(chat_id=chat_id, text="*✅ THE END ✅*\n*WELCOME BACK✨✨🤗!*", parse_mode='Markdown')
...
async def attack(update: Update, context: CallbackContext):
    ...
    if user_id not in users or get_remaining_approval_time(user_id) == "Expired":
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **JO KAL TAK HAMARE DUM PE UDTE THE ! VO AAJ MUJHE AASMAN KE BAARE ME BATA RAHE HAI 😂** .*", parse_mode='Markdown')
        return

    if attack_in_progress:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ **ATTACK IS RUNNING WAIT**.*", parse_mode='Markdown')
        return
